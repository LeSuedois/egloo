...
const SERVICE_UUID = 'd91cb6ee-d174-11ea-87d0-0242ac130004'

const EGLOO_INPUTS = 'EglooInputs'
const EGLOO_OUTPUTS = 'EglooOutputs'
const EGLOO_NEOPIXELS = 'EglooNeopixels'
...
class Nano33BLE extends EventEmitter {
	constructor(options = {}) {
		...
		this.characteristics = {
			[EGLOO_INPUTS]: {
				uuid: 'd91cb6ee-5001-11ea-87d0-0242ac130004',
				properties: ['BLENotify', 'BLEWrite'],
				structure: ['Int16', 'Int16', 'Int16', 'Int16', 'Int16'],
				data: { i1: [], i2: [], i3: [], i4: [], i5: [] }
			},
			[EGLOO_OUTPUTS]: {
				uuid: 'd91cb6ee-5002-11ea-87d0-0242ac130004',
				properties: ['BLEWrite'],
				structure: ['Int8', 'Int8', 'Int8', 'Int8', 'Int8'],
				data: { o1: [], o2: [], o3: [], o4: [], o5: [] },
				writeBusy: false, // we need to track this to avoid 'GATT operation in progress' errors
    			writeValue: null
			},
			[EGLOO_NEOPIXELS]: {
				uuid: 'd91cb6ee-5003-11ea-87d0-0242ac130004',
				properties: ['BLEWrite'],
			...
			[CORUI]: {
				...
				properties: ['BLENotify'],//, 'BLEWrite'],
				structure: ['Int16', 'Int16', 'Int16', 'Int16'],
				data: { ca0: [], ca1: [], ca2: [], cb: [] }
			},
			[CORLED]: {
	...
	connect = async () => {
		...
		for (const sensor of this.sensors) {
			...
			//if (!["BolLed", "BolUI", "CorLed", "CorUI"].includes(sensor)) continue // use hard-coded sensor names
			try {
				this.characteristics[
					sensor
	...
	BLEwriteTo = (sensor) => {
		console.log(this.characteristics[sensor].characteristic);
		if (this.characteristics[sensor].writeBusy) return; // dropping writes when one is in progress instead of queuing as LED is non-critical / realtime
		this.characteristics[sensor].writeBusy = true; // Ensure no write happens when GATT operation in progress
		this.characteristics[sensor].characteristic.writeValue(this.characteristics[sensor].writeValue)
		.then(_ => {
			this.characteristics[sensor].writeBusy = false;
		})
		.catch(error => {
		  console.log(error);
		});
	...
	writeBolLed = (rgb_values) => {
		this.characteristics.BolLed.writeValue = rgb_values;
		this.BLEwriteTo(BOLLED);
	}
	writeCorLed = (rgb_values) => {
		this.characteristics.CorLed.writeValue = rgb_values;
		this.BLEwriteTo(CORLED);
	}
	writeNeopixels = (rgb_values) => {
		this.characteristics.EglooNeopixels.writeValue = rgb_values;
		this.BLEwriteTo(EGLOO_NEOPIXELS);
	}
	writeOutputs = (output_values) => {
		this.characteristics.EglooOutputs.writeValue = output_values;
		this.BLEwriteTo(EGLOO_OUTPUTS);
	}

	

	static get EGLOO_INPUTS() {
		return EGLOO_INPUTS
	}
	static get EGLOO_OUTPUTS() {
		return EGLOO_OUTPUTS
	}
	static get EGLOO_NEOPIXELS() {
		return EGLOO_NEOPIXELS
	}
